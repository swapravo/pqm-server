...
def username_availability_check(connection, request):
	...
	print("Processed a signup request!")
...
def process(connection):
	...
	while True:
		...
		try:
			print(request)
			if not (isinstance(request["version"], str) and
				(request["token"]["type"] == "hash" or
				request["token"]["type"] == "sign") and
				isinstance(request["request"], bytes)):
				src.network.block(connection, src.globals.HOUR, block_ip_plus_port=True)
				return
				# kill process here
		...
		request = src.crypto.asymmetrically_decrypt(request["request"], src.globals.SERVER)
